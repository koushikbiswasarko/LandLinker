...
<div class="dash">
  <aside class="dash-side" aria-label="Dashboard sidebar">
    ...
    <nav class="dash-nav" aria-label="Dashboard navigation">
      <a class="dash-link <?= (($activeDash ?? '') === 'dashboard') ? 'is-active' : ''; ?>" href="<?= url('pages/manager-dashboard.php') ?>">
        <span class="dash-ico" aria-hidden="true">🏠</span>
        <span>Dashboard</span>
      </a>

    <a class="dash-link <?= ($activeDash === 'employee') ? 'is-active' : '' ?>"
    href="<?= url('pages/manager/users.php') ?>">
    <span class="dash-ico">👥</span>
    <span>Users</span>
    </a>


      <a class="dash-link <?= (($activeDash ?? '') === 'activities') ? 'is-active' : ''; ?>" href="<?= url('pages/employee-tasks.php') ?>">
        <span class="dash-ico" aria-hidden="true">📌</span>
        <span>Activities</span>
      </a>

      <a class="dash-link <?= (($activeDash ?? '') === 'deal_stats') ? 'is-active' : ''; ?>" href="<?= url('pages/deal-statistics.php') ?>">
        <span class="dash-ico" aria-hidden="true">📈</span>
        <span>Deal Stats</span>
      </a>

      <a class="dash-link <?= (($activeDash ?? '') === 'profile') ? 'is-active' : ''; ?>" href="<?= url('pages/profile.php') ?>">
        <span class="dash-ico" aria-hidden="true">👤</span>
        <span>Profile</span>
      </a>
    </nav>
